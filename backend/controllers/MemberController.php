...
//            Yii::$app->getSession()->setFlash('error','您还未登记任何车辆，请先登记车辆信息！');
    ...
    public function actionSure_to_add(){
        if(Yii::$app->request->post()){
            ...
            $new_order = new Orders();
            $new_order->order_no = $order_no;
            $new_order->service_id = $reason_order['service_id'];
            $new_order->service_name = $reason_order['service_name'];
            $new_order->create_time = time();
            ...
            endforeach;
            if(Additional_goods::deleteAll(['reason_id' => $reason_id]) && $reason->save()){
                echo 111;
            }
            exit;
        }
